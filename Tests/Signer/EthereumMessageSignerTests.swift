//
//  EthereumMessageSignerTests.swift
//  AppChainTests
//
...
class EthereumMessageSignerTest: XCTestCase {
    func testSignMessage() {
        ...
        guard let sign = try? EthereumMessageSigner().sign(message: data, privateKey: privateKey, useExtraEntropy: false) else {
    ...
    func testSignPersonalMessage() {
        let privateKey = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
        let data = "This is a sweet message".data(using: .utf8)!
        guard let sign = try? EthereumMessageSigner().signPersonalMessage(message: data, privateKey: privateKey, useExtraEntropy: false) else {
    ...
    func testHashMessage() {
        ...
        for (data, hash) in fixtures {
            XCTAssertEqual(hash, EthereumMessageSigner().hashMessage(data).toHexString().addHexPrefix())
        }
    }
    ...
    func testHashPersonalMessage() {
        ...
        for (data, hash) in fixtures {
            XCTAssertEqual(hash, EthereumMessageSigner().hashPersonalMessage(data)!.toHexString().addHexPrefix())
        }
    }
}
