...
class RPCTests: XCTestCase {
    ...
    override class func setUp() {
        ...
        do {
            ...
            txHashRecentSent = try appChain.rpc.sendRawTransaction(signedTx: signed)
        } catch {
            txHashRecentSent = ""
        }
    ...
    func testGetBlockByHash() throws {
        let block = try appChain.rpc.getBlockByNumber(number: BigUInt(try appChain.rpc.blockNumber()), fullTransactions: true)
        let hash = block.hash.toHexString().addHexPrefix()
        let query = try appChain.rpc.getBlockByHash(hash: hash, fullTransactions: true)
    ...
    func testGetBlockByNumber() throws {
        let currentBlock = try appChain.rpc.blockNumber()
        let block = try appChain.rpc.getBlockByNumber(number: BigUInt(currentBlock), fullTransactions: true)
        XCTAssertEqual(block.header.number, BigUInt(currentBlock))
    }

    func testGetTransactionReceipt() throws {
        let receipt = try appChain.rpc.getTransactionReceipt(txhash: RPCTests.txHashRecentSent!)
    ...
    func testGetTransactionProof() throws {
        let proof = try appChain.rpc.getTransactionProof(txhash: RPCTests.txHashRecentSent!)
