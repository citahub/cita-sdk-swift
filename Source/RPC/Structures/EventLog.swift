...
public struct EventLog: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let data = try DecodeUtils.decodeHexToData(container, key: .data) else { throw AppChainError.dataError }
        self.data = data

        guard let logIndex = try DecodeUtils.decodeHexToBigUInt(container, key: .logIndex) else { throw AppChainError.dataError }
        ...
        for top in topicsStrings {
            guard let topic = Data.fromHex(top) else { throw AppChainError.dataError }
